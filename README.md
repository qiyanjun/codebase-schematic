...
3. **Designs a diagram that argues that fact**, using a small, deliberate visual-pattern library (fan-out, layers, timeline, tree, convergence) and a fixed semantic color palette.
...
**The isomorphism test**: strip every label from the diagram — does the shape and arrangement alone still communicate the argument? If a diagram only makes sense with its labels intact, it's a list wearing a diagram's clothes.
...
installed, invoke as `/codebase-schematic` or, namespaced, as
`/codebase-schematic:codebase-schematic`.
